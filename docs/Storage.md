...
encoding and stored across multiple **backends**. The Objects are split in a way that guarantees a variable amount of backend failures before data is lost. 

## Chunks
A chunk is a contiguous segment of an S3-Object. Chunks are the units of data that are split into shards and distributed across backends.

## Shards
A shard contains a part of a chunk and is stored on a backend. The shards are encoded with **reed-solomon** encoding to allow for backend failures.  
Shards can be combined into a blob for a single backend to reduce the number of api calls to each backend. The maximum size of a blob is determined by the maximum file size of the backend.

## Backends
Backend instances are declared in `backends.yaml` (path configurable via `SHARDS3_BACKENDS_CONFIG_PATH`):  
...
## Metadata
Metadata is stored in a simple [SQLite Database](./Metadata.md). The metadata contains information about each S3-Object as well as information about each **chunk** and **shard**.

## Chunking and Sharding

## Example
The following diagram shows how an S3-Objects is split and redundantly stored.  
...
The sharding is calculated automatically by an algorithm in the `storage` module. 
